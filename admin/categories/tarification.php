    ...
    <div class="admin-wrapper">
        ...
        <main class="main-content">
            <div class="container-fluid py-4">
                ...
                // Fonction de validation avant l'ajout
                function validatePricing(jsonData) {
                    return new Promise((resolve, reject) => {
                        // Vérifier uniquement la combinaison catégorie + niveau d'étude
                        fetch(`check_existing_pricing.php?categorie_id=${jsonData.categorie_id}&niveau_etude=${encodeURIComponent(jsonData.niveau_etude)}`)
                            .then(response => response.json())
                            .then(data => {
                                if (data.exists) {
                                    reject('Un tarif existe déjà pour cette catégorie avec ce niveau d\'étude. Veuillez modifier le tarif existant ou choisir une autre combinaison.');
                                } else {
                                    resolve(true);
                                }
                            })
                            .catch(error => {
                                ...
                                reject('Erreur lors de la vérification du tarif');
                            });
                    });
                }

                // Modifier le gestionnaire de formulaire
                document.getElementById('pricingForm').addEventListener('submit', function(e) {
                    ...
                    // Vérifier les doublons avant l'ajout
                    validatePricing(jsonData)
                        .then(() => {
                            // Procéder à l'ajout
                            return fetch('add_pricing.php', {
                                method: 'POST',
                                headers: {
                        ...
                        .then(data => {
                            if (data.success) {
                                ...
                                Swal.fire({
                                    icon: 'success',
                                    title: 'Succès',
                                    text: 'Tarification ajoutée avec succès',
                                    toast: true,
                                    position: 'top-end',
